...
import { Alert, Select, Table } from "@mantine/core";
...
import type { SubCategoryOrdersSort } from "./sort";

export default function SubCategoryOrdersTable({
  data,
  sort,
  onSortChange,
}: {
  data: PurchasingMaterialsCategoryOrder[];
  sort: SubCategoryOrdersSort;
  onSortChange: (sort: SubCategoryOrdersSort) => void;
}) {
    ...
    <ReportCard
      ...
      headerAction={
        <Select
          value={sort}
          onChange={(value) => {
            if (value) onSortChange(value as SubCategoryOrdersSort);
          }}
                  ...
                  <Table.Tr key={row.orderId} className="text-gray-600">
                    ...
                    <Table.Td>
                      {row.invoiceNumbers.length > 0 ? (
                        <div className="flex items-center gap-1.5">
                          <span className="font-mono text-gray-600">{row.invoiceNumbers.join(", ")}</span>
                          <CopyButton text={row.invoiceNumbers.join(", ")} />
                        </div>
                      ) : (
