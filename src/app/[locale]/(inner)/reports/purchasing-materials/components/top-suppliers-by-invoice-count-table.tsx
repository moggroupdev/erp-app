...
import type { PurchasingMaterialsSupplierInvoiceCount } from "@/types/reports";
import ReportCard from "./report-card";

export default function TopSuppliersByInvoiceCountTable({
  data,
}: {
  data: PurchasingMaterialsSupplierInvoiceCount[];
}) {
            ...
            <Table.Tbody>
              {data.map((row, index) => (
                <Table.Tr key={row.supplierId} className="text-gray-600">
                  ...
                  <Table.Td className="font-semibold text-gray-800">{row.invoiceCount}</Table.Td>
                  <Table.Td>{formatMoney(row.totalSpend)}</Table.Td>
                </Table.Tr>
              ))}
            </Table.Tbody>
          </Table>
