...
import { ArrowUp, ClipboardList, Package, ShoppingCart, Truck } from "lucide-react";
import { useI18n } from "@/lib/i18n/hooks";

export default function RequisitionFollowUpEmpty() {
  const { translate } = useI18n();

  const hints = [
    {
      icon: ClipboardList,
      label: translate("Approved requisition lines", "بنود طلبات الشراء المعتمدة"),
      detail: translate(
        "All fully approved lines for the selected production department",
        "جميع البنود المعتمدة بالكامل لقسم الإنتاج المحدد",
      ),
    },
    {
      icon: Package,
      label: translate("Requested quantity", "الكمية المطلوبة"),
      detail: translate("Quantity requested on each requisition line", "الكمية المطلوبة على كل بند في طلب الشراء"),
    },
    {
      icon: ShoppingCart,
      label: translate("Ordered", "تم الطلب"),
      detail: translate(
        "Quantity already allocated to material purchase orders",
        "الكمية المخصصة بالفعل على أوامر توريد الخامات",
      ),
    },
    {
      icon: Truck,
      label: translate("Received", "تم الاستلام"),
      detail: translate(
        "Accepted goods-receipt quantity prorated to each requisition line",
        "كمية الاستلام المقبولة موزّعة تناسبياً على كل بند طلب شراء",
      ),
    },
  ];
