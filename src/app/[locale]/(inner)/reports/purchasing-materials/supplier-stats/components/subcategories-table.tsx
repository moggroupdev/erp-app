...
import type { PurchasingMaterialsSupplierBySubCategory } from "@/types/reports";
import ReportCard from "../../components/report-card";
import type { SupplierSubCategoriesSort } from "./sort";

export default function SupplierSubCategoriesTable({
  data,
  sort,
  onSortChange,
}: {
  ...
  onSortChange: (sort: SupplierSubCategoriesSort) => void;
}) {
  ...
  return (
    <ReportCard
      ...
      description={translate(
        "Purchase value from this supplier broken down by material subcategory.",
        "قيمة المشتريات من هذا المورد مفصّلة حسب الفئة الفرعية للمواد.",
      )}
      icon={Layers}
      accent="teal"
      headerAction={
        <Select
          value={sort}
          onChange={(value) => {
            if (value) onSortChange(value as SupplierSubCategoriesSort);
          }}
          ...
          data={[
            ...
            { value: "main-asc", label: translate("Main category (A-Z)", "الفئة الرئيسية (أ-ي)") },
            { value: "main-desc", label: translate("Main category (Z-A)", "الفئة الرئيسية (ي-أ)") },
          ]}
          allowDeselect={false}
          radius="md"
          w={240}
            ...
            <Table.Tbody>
              {data.map((row, index) => (
                <Table.Tr key={row.subCategoryId} className="text-gray-600">
                  ...
                  <Table.Td className="max-w-[220px] truncate font-medium text-gray-800">
                    <Link
                  ...
                  <Table.Td className="max-w-[220px] truncate font-medium text-gray-800" title={row.subCategoryTitle}>
