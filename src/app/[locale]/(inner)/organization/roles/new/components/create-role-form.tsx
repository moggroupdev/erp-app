...
import { Check, Info, KeyRound, Shield, X } from "lucide-react";
...
export default function CreateRoleForm() {
  ...
  const [maxDiscountPct, setMaxDiscountPct] = useState<number | string>("");
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!name.trim()) {
      return setValidationError(translate("Please enter the role name.", "يرجى إدخال اسم الدور."));
    }

    if (isSalesDepartment && maxDiscountPct !== "" && (Number(maxDiscountPct) < 0 || Number(maxDiscountPct) > 100)) {
      return setValidationError(translate("Max discount must be between 0 and 100.", "يجب أن يكون أقصى خصم بين 0 و 100."));
    }
  ...
  function toggleDomainPermissions(domainPermissions: Permission[]) {
    ...
    if (allDomainSelected) {
      setPermissions(permissions.filter((p) => !domainPermissions.includes(p)));
    } else {
      setPermissions([...new Set([...permissions, ...domainPermissions])]);
    }
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <div key={group.domain} className="flex flex-col gap-2 rounded-2xl bg-slate-50/75 p-4">
                    <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2 border-b border-dashed border-gray-200 pb-2">
                      <div className="flex items-center gap-2">
                        <h4 className="text-sm font-semibold text-gray-800">{translate(group.label.en, group.label.ar)}</h4>
                        <span
      ...
      <div className="flex flex-col gap-3 md:flex-row md:justify-between">
        ...
        <div className="flex flex-col gap-2 md:flex-row">
          ...
          <Button type="submit" loading={mutation.isPending} disabled={!name.trim()} radius="md">
            {translate("Create Role", "إنشاء الدور")}
          </Button>
        </div>
      </div>
