...
export function formatDimensionLabelText(dimension: DimensionLike, unit?: string): string {
  if (dimension.diameter != null) return appendUnit(`⌀${dimension.diameter} × ${dimension.height}`, unit);
  return appendUnit(`${dimension.length} × ${dimension.depth} × ${dimension.height}`, unit);
}
  ...
  if (dimension.diameter != null) {
    return (
      <span className="inline-flex items-center gap-0.5">
        ...
        {appendUnit(`${dimension.diameter} × ${dimension.height}`, unit)}
      </span>
    );
  }
