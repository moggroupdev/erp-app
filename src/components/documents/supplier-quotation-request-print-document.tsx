...
export default function SupplierQuotationRequestPrintDocument({
  ...
  const contactLines = [
    preparedBy.name,
    preparedBy.email ? `${translate("Email", "البريد الإلكتروني")}: ${preparedBy.email}` : null,
    preparedBy.phone ? `${translate("Phone", "الهاتف")}: ${preparedBy.phone}` : null,
    ...
    <div className="flex flex-col gap-5 text-xs text-gray-900">
      <header className="flex items-start justify-between gap-4 border-b border-gray-300 pb-4">
        <div className="flex min-w-0 flex-col gap-1">
          <p className="text-[10px] font-medium tracking-wide text-gray-500 uppercase">
            {translate("MOG - 10th of Ramadan Catering Equipment", "مصانع العاشر من رمضان لمعدات التموين (MOG)")}
          </p>
      ...
      <section className="flex flex-col gap-3 text-[11px] leading-relaxed">
        ...
        <p>
          ...
          {translate(
            "Request for Quotation for the Supply of Materials",
            "طلب عرض أسعار لتوريد أصناف",
          )}
        </p>
      ...
      <footer className="mt-4 flex flex-col gap-6 break-inside-avoid pt-2">
        <p className="text-[11px]">
          {translate(
            "We look forward to receiving your quotation and thank you for your cooperation.",
            "نتطلع إلى تلقي عرضكم، مع خالص الشكر والتقدير لتعاونكم.",
          )}
        </p>
        <p className="text-[11px]">{translate("Yours sincerely,", "وتفضلوا بقبول فائق الاحترام والتقدير،")}</p>
        <div className="flex flex-col gap-1">
          <p className="text-[10px] font-medium tracking-wide text-gray-500 uppercase">
            {translate("Prepared by", "أعد بواسطة")}
          </p>
          {contactLines.map((line) => (
            <p key={line} className="text-[11px] text-gray-900">
              {line}
            </p>
          ))}
